...
Everything else is open. Talk it through on GitHub issues. Pull requests are welcome. Harden what already ships if you can (`STRENGTHEN.md`, `just test`, `ARCHITECTURE.md`); fork and tweak workflows if your household needs something different. Contributor linting requires Node.js with `npx`. `just setup` (or `just hooks`) installs local git hooks: **pre-commit** runs `just lint` (Markdown + `fmt` + `clippy`), **pre-push** runs `just test`.
