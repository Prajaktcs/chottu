...
| **Coordinator** | `coordinator` | Selected Signal or Telegram chat transport, OAuth localhost callback, morning brief, evening reflection, evening `/networth` overview, task due pings |
...
Agents persist through SQLite (and files under `CHOTU_BRAIN_DIR` / `~/chotu_brain`). In-process work uses cloned `SqlitePool`, one shared `ChatClient`, plus an immutable `Arc<AppConfig>`; provider-scoped authorization config is loaded once and changes require restart.

```text
Selected chat / IMAP / drop folder / Google APIs
                 │
                 ▼
        coordinator binary (supervisor)
   ┌─────────┬──────────┬──────────────┬─────────────┐
   │streamer │ janitor  │ health-coach │ chat +      │
   │         │          │              │ schedulers  │
   └────┬────┴────┬─────┴──────┬───────┴──────┬──────┘
        │         │            │              │
        └─────────┴────────────┴──────────────┘
...
| **Gemini** (`GEMINI_API_KEY` — **required by `just run`**) | Food photos (package/plate), PDF ingest, unstructured nutrition estimates |
...
## Family isolation & chat

- Roster, `nutrition_goals`, `fitness_goals`, `core_values`, `spend_budgets`, investment philosophy, and provider-scoped chat IDs live in **gitignored** `config.yaml`.
- `CHOTU_CHAT_PROVIDER` selects Signal (the default) or Telegram. Direct messages require an exact `family.members[].chat_ids.<provider>` match; groups require the exact `chat.household_ids.<provider>` match. There is no open bootstrap or runtime identity mutation.
...
- The optional household conversation is the family-wide surface. Proactive fan-out sends schedules, budget alerts, and research reports to configured member DMs plus that household conversation. Personal health notifications remain DM-only.
- The receive loop drains normalized messages into provider-qualified per-conversation queues. One worker serializes each conversation, while a global semaphore bounds concurrent handlers.

---

## Health & day loop (implementation notes)

- Chat `/food` (and photos) resolves relative days and **meal windows** (breakfast / lunch / snacks / dinner) in `chotu-common` before logging; Google Health meal labels use matching hour buckets.
...
- Tasks: email-derived or chat-created; dated tasks can write Google Calendar; complete/snooze keep the event in sync; open/snoozed lists and due reminders include `/tasks complete <id>` and `/tasks snooze <id> [days]`.
