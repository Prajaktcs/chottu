...
are managed from chat, so they are DB rows (below). Condition definitions
change ~yearly.
...
-- Per-member, per-condition watchlist (managed via the /watch chat command).
...
| **M5** | `/watch` + `/tags` chat commands | M1 |
